...
- [`usage`](usage.md) - report what the analysed project's own code uses from one dependency
...
- [`vuln-scan-rescan`](rescan.md) - re-scan an existing walk against a fresh vulnerability snapshot
...
- [`native-list`](native.md#native-list---reading-the-records-back-in-bulk) - list the modules examined for native code, filterable by what was found
