...
// Bumped to "0.7.0" when a position stopped naming the host that ran the
// analysis. A symbol generated by cgo is declared in a file the toolchain writes
// into the content-addressed build cache, and the record stored that path: a
// place only the analysing host had, under a directory whose name changes
// whenever the entry is rebuilt. Two analyses of one module differing in nothing
// but GOCACHE recorded 99 of github.com/mattn/go-sqlite3@v1.14.12's nodes at
// different files — same cache entry, same line, different cache root — and
// composed to a refusal. At most one of such a pair can be true and neither is
// checkable by anyone else, which is the criterion at the top of this comment.
// The stdlib and module-cache paths beside them named the host in the same way:
// 180 of cobra@v1.10.2's 226 distinct files were absolute paths with the leading
// separator stripped, so they read as relative and were not.
//
// Measured on this store: 275 of the 597 coordinates at 0.6.0 carry a
// non-external build-cache position, four had already recorded divergence and
// six refuse outright. Without the bump a re-derivation lands beside the old
// generation at the same completeness and composition — correctly — reports two
