...
// qualifies a failure, and a record carrying a graph outranks any failure
// regardless of what caused it. What it decides is cache eligibility — see
// RecordIsCacheable.
...
//
//   - A record that FAILED and states no cause is not cacheable. It predates the
//     axis, so nothing about it says the module was at fault, and treating "we
//     do not know" as "the module is broken" is the exact collapse this type
//     exists to prevent. It costs one re-attempt per such record, once: the
//     re-attempt writes a record that does state its cause, and from then on the
//     coordinate settles either way. Records that did not fail are unaffected —
//     they are the overwhelming majority, they keep answering, and no purge or
//     pipeline-version bump is owed for any of this.
//
...
// ExcludedByConfig is absent too — a module the operator chose not to analyse is
// a decision, not a failure, and re-attempting it every run would ignore the
// decision.
