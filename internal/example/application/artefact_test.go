...
import (
	...
	domain3 "github.com/eitanity/kanonarion/internal/example/domain"
...
func TestExecute_RecordsTheArtefactItWasDerivedFrom(t *testing.T) {
	...
	stored, found, err := exampleStore.GetExampleRecord(context.Background(), coord, application.PipelineVersion)
	if err != nil || !found {
		t.Fatalf("GetExampleRecord: found=%v err=%v", found, err)
	}
...
func TestExecute_FailedExtractionStillNamesTheArtefact(t *testing.T) {
	...
	if result.Record.OverallStatus != domain3.ExampleStatusExtractionFailed {
