...
func TestLedger_IdenticalGenerationFindsTheMeasurementAlreadyHeld(t *testing.T) {
	...
	held := ledgerRecord(t, local, domain2.ExampleStatusFound, []string{"ExampleA"}, 0, first, artefact)
	if perr := s.PutExampleRecord(ctx, held); perr != nil {
	...
	fresh := ledgerRecord(t, local, domain2.ExampleStatusFound, []string{"ExampleA"}, 0, first.Add(time.Hour), artefact)
...
// prefilter cannot decide on its own. Both generations state one example, so
// every column the table records agrees; only the example inside the record says
...
func TestLedger_IdenticalGenerationRefusesADifferentAnalysis(t *testing.T) {
	...
	artefact := fetchtest.ZipArtefact("tree-one=").String()
	first := time.Date(2026, 3, 1, 0, 0, 0, 0, time.UTC)
	if perr := s.PutExampleRecord(ctx,
		ledgerRecord(t, local, domain2.ExampleStatusFound, []string{"ExampleA"}, 0, first, artefact)); perr != nil {
		t.Fatalf("PutExampleRecord: %v", perr)
	}

	// A different tree, an example set of the same size.
	other := ledgerRecord(t, local, domain2.ExampleStatusFound, []string{"ExampleA"}, 0,
	...
	// The same artefact, a renamed example. The example count and the status are
	// unchanged, so a comparison over columns would call these one measurement.
	renamed := ledgerRecord(t, local, domain2.ExampleStatusFound, []string{"ExampleB"}, 0, first.Add(2*time.Hour), artefact)
	if _, found, gerr := s.IdenticalGeneration(ctx, renamed); gerr != nil || found {
		t.Fatalf("a different example matched a held generation: found=%v err=%v", found, gerr)
	}
}
...
// served, and two of them state one example set for one coordinate with nothing
// to show they describe the same bytes.
func TestLedger_IdenticalGenerationMatchesNothingWhenTheAnalysisIsUnnamed(t *testing.T) {
	...
	seedPreLedgerRow(t, s.InternalDB(),
		ledgerRecord(t, local, domain2.ExampleStatusFound, []string{"ExampleA"}, 0, first, ""))

	unnamed := ledgerRecord(t, local, domain2.ExampleStatusFound, []string{"ExampleA"}, 0, first.Add(time.Hour), "")
...
func TestLedger_IdenticalGenerationRefusesTheZeroCoordinate(t *testing.T) {
	s := openTestStore(t)
	_, _, err := s.IdenticalGeneration(context.Background(), domain2.ExampleRecord{
...
func TestLedger_IdenticalGenerationReportsAnUnverifiableRowRatherThanAgreement(t *testing.T) {
	...
	if _, uerr := s.InternalDB().DB().Exec(
		`UPDATE example_records SET serialised = ?`, blobcodec.Encode(blob)); uerr != nil {
		t.Fatalf("tampering with the stored row: %v", uerr)
	}

	fresh := ledgerRecord(t, local, domain2.ExampleStatusFound, []string{"ExampleA"}, 0, first.Add(time.Hour), artefact)
	_, found, gerr := s.IdenticalGeneration(ctx, fresh)
	if !errors.Is(gerr, ports.ErrExampleIntegrity) {
		t.Fatalf("err = %v, want %v", gerr, ports.ErrExampleIntegrity)
	}
	if found {
