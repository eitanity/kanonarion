...
// SameMeasurement reports whether two example records state the identical
...
// that came back saying the same examples, the same parse failures and the same status has added no fact — the only thing new about it
...
func SameMeasurement(a, b ExampleRecord) (bool, error) {
	...
	ab, err := marshalCanonicalExample(forMeasurementComparison(a))
	if err != nil {
		return false, fmt.Errorf("marshal example record for measurement comparison: %w", err)
	}
	bb, err := marshalCanonicalExample(forMeasurementComparison(b))
	if err != nil {
		return false, fmt.Errorf("marshal example record for measurement comparison: %w", err)
	}
...
func forMeasurementComparison(r ExampleRecord) ExampleRecord {
