...
//
// It is a free function rather than a method because ExampleRecord is aliased into
// pkg/kanonarion as a result type, and result types are read shapes that must
// not grow behaviour.
func RecordArtefactIdentity(r ExampleRecord) (fetchdomain.ArtefactIdentity, error) {
	id, err := fetchdomain.StoredArtefactIdentity(r.ArtefactIdentity)
	if err != nil {
		return fetchdomain.ArtefactIdentity{}, fmt.Errorf("reading the artefact identity of the example record for %s: %w", r.Coordinate, err)
	}
	return id, nil
}
...
func NamesAnalysedContent(r ExampleRecord) bool {
