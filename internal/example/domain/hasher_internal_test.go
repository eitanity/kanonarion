...
func TestMarshalCanonicalExample_MarshalFailure(t *testing.T) {
	...
	_, err := ExampleRecordHasher{}.SetContentHash(ExampleRecord{})
	if err == nil {
	...
	if !strings.Contains(err.Error(), "canonical example record") {
...
func TestVerifyContentHash_MarshalFailure(t *testing.T) {
	...
	err := ExampleRecordHasher{}.VerifyContentHash(ExampleRecord{})
	if !errors.Is(err, injected) {
