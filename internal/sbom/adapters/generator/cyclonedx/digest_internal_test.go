...
func TestBuildDependencies(t *testing.T) {
	...
	deps := buildDependencies(components, root, graph, subject{target: target, ref: moduleRef(target)}, nil)
	...
	if got := buildDependencies(components, nil, graph, subject{target: target, ref: moduleRef(target)}, nil); len(got) != 2 {
