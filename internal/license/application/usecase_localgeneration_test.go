...
// localLicenceUseCase wires the stage over a local coordinate whose tree has
// already been ingested.
func localLicenceUseCase(t *testing.T) (
	...
	*fakeBlobStore,
	...
	ingestLocalTree(t, facts, blobs, coord, "zip-one", map[string]string{"LICENSE": "MIT License text"})
	...
	return uc, store, detector, facts, blobs, coord
}
...
// states, and appending it again writes a full record to say nothing new.
func TestExecute_ReExtractingAnUnchangedLocalTreeAppendsNothing(t *testing.T) {
	uc, store, detector, _, _, coord := localLicenceUseCase(t)
	...
	if detector.calls < 2 {
		t.Fatalf("the detector ran %d times; a local tree must be re-read on every run", detector.calls)
	}
	if !second.Reused {
...
// project's own licence permanently stale.
func TestExecute_AChangedLocalTreeIsStillAppended(t *testing.T) {
	uc, store, detector, facts, blobs, coord := localLicenceUseCase(t)

	if _, err := uc.Execute(context.Background(), application.ExtractRequest{Coordinate: coord}); err != nil {
		t.Fatalf("Execute: %v", err)
	}

	// The project relicensed: the LICENSE file changed, so the tree re-zips to
	// different bytes and the detector now reads a different grant.
	detector.match = ports.LicenseMatch{SPDX: "Apache-2.0", Confidence: 0.99}
	ingestLocalTree(t, facts, blobs, coord, "zip-two", map[string]string{"LICENSE": "Apache License text"})
	...
	if second.Record.PrimarySPDX != "Apache-2.0" {
		t.Errorf("PrimarySPDX = %q, want Apache-2.0: the new measurement was not served", second.Record.PrimarySPDX)
	}
	if len(store.puts) != 2 {
...
// caller re-measures because something OUTSIDE the tree changed — a corrected
// detector, a rebuilt licence corpus — and asks for the result to be recorded.
// Collapsing it into the held generation would leave that caller with no way to
// record a measurement at all.
func TestExecute_ForceAppendsALocalGenerationEvenWhenIdentical(t *testing.T) {
	uc, store, _, _, _, coord := localLicenceUseCase(t)
...
// stated rather than swallowed. Losing a measured licence to protect an
...
// plainLicenceStore is a store that does not offer IdenticalGeneration: every
...
type plainLicenceStore struct{ inner *fakeLicenseStore }

func (s plainLicenceStore) PutLicenseRecord(ctx context.Context, r licensedomain.LicenseRecord) error {
	return s.inner.PutLicenseRecord(ctx, r)
}

func (s plainLicenceStore) GetLicenseRecord(
	ctx context.Context, coord coordinate.ModuleCoordinate, pv string,
) (licensedomain.LicenseRecord, bool, error) {
	return s.inner.GetLicenseRecord(ctx, coord, pv)
}

func (s plainLicenceStore) ListLicenseRecords(ctx context.Context, f ports.LicenseFilter) ([]ports.LicenseSummary, error) {
	return s.inner.ListLicenseRecords(ctx, f)
}
...
func TestExecute_AStoreWithoutTheCapabilityStillRecordsEveryRun(t *testing.T) {
	...
	facts := &fakeFactStore{}
	blobs := &fakeBlobStore{}
	inner := &fakeLicenseStore{}
	ingestLocalTree(t, facts, blobs, coord, "zip-one", map[string]string{"LICENSE": "MIT License text"})

	uc := application.NewExtractLicenseUseCase(application.Config{
		Facts:     facts,
		Blobs:     blobs,
		Licenses:  plainLicenceStore{inner: inner},
		Detector:  &fakeDetector{match: ports.LicenseMatch{SPDX: "MIT", Confidence: 0.98}},
		Clock:     &advancingClock{t: time.Date(2026, 6, 1, 0, 0, 0, 0, time.UTC)},
		Stopwatch: fakeStopwatch{},
		Logger:    slog.New(slog.NewTextHandler(io.Discard, nil)),
