...
func TestExecute_AConflictingLedgerIsACacheMissNotAFailure(t *testing.T) {
	...
	licences := &fakeLicenseStore{
		getErr: fmt.Errorf("%w: two generations", ports.ErrLicenceConflict),
	}

	uc := buildUseCase(t, facts, blobs, licences)
	...
	if _, ok := licences.records[licenseKey{coord.Path(), coord.Version(), application.PipelineVersion}]; !ok {
...
func TestExecute_AStoreFailureIsStillAnExtractionFailure(t *testing.T) {
	...
	uc := buildUseCase(t, facts, blobs, &fakeLicenseStore{getErr: unreadable})
