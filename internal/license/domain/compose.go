...
// SameMeasurement reports whether two licence records state the identical
...
// that came back saying the same licence files, the same expression, the same status and the same
// provenance has added no fact — the only thing new about it
...
func SameMeasurement(a, b LicenseRecord) (bool, error) {
	...
	ab, err := marshalCanonicalLicense(forMeasurementComparison(a))
	if err != nil {
		return false, fmt.Errorf("marshal licence record for measurement comparison: %w", err)
	}
	bb, err := marshalCanonicalLicense(forMeasurementComparison(b))
	if err != nil {
		return false, fmt.Errorf("marshal licence record for measurement comparison: %w", err)
	}
...
func forMeasurementComparison(r LicenseRecord) LicenseRecord {
