...
func TestRefusesZeroArtefactIdentity(t *testing.T) {
	s := openTestStore(t)
	rec := buildRecord(t, mustCoord(t, "example.com/mod", "v1.0.0"), "MIT", domain2.LicenseStatusDetected)
	rec.ArtefactIdentity = ""

	fetchtest.AssertRefusesZeroIdentity(t, "PutLicenseRecord", func() error {
		return s.PutLicenseRecord(context.Background(), rec)
	})
}
