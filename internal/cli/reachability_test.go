...
var reachCoord = coordinatetest.MustNew("golang.org/x/text", "v0.3.7")
...
func TestVulnReachabilityVerdict_ConfidentAnswers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			res, err := vulnReachabilityVerdict(reachCoord, tt.rec, true, tt.vulnID, nil, nil)
			if err != nil {
...
func TestVulnReachabilityVerdict_Unresolved(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := vulnReachabilityVerdict(reachCoord, tt.rec, tt.found, "GO-2021-0113", nil, nil)
			if err == nil {
