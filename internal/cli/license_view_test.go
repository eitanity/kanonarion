...
// TestLicenseViewIsATotalProjection is the structural half, and it is what stops
// the surface regrowing at a level nobody is reading.
//
// A domain struct anywhere in the view republishes ITS field names, and every
// field of every type below it, so the view can be correct at the top and still
// be one field away from being the contract again. The walk is over the whole
// reachable type graph, not the top level, because that is exactly the mistake
// the first attempt at this made.
func TestLicenseViewIsATotalProjection(t *testing.T) {
	// domain.Obligations is the one domain type this surface publishes directly,
	// and it is admitted by NAME because it carries explicit snake_case tags of
	// its own — it is already a wire type, not a Go one.
	tagged := reflect.TypeOf(domain.Obligations{})
	...
	// string. The walk stops there rather than reporting the unexported innards
	// of time.Time as an unnamed surface.
	...
	walk = func(rt reflect.Type, at string) {
		...
		if rt.Kind() != reflect.Struct || seen[rt] || rt == tagged || selfEncoding(rt) {
			return
		}
		seen[rt] = true
		if strings.HasSuffix(rt.PkgPath(), "/license/domain") {
			t.Errorf("%s reaches the domain type %s: an embedded or named domain struct publishes its own "+
				"field names, which is the link this view exists to cut", at, rt.Name())
			return
		}
	...
	walk(reflect.TypeOf(licenseDocument{}), "licenseDocument")
}
