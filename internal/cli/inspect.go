...
func runInspect(ctx context.Context, arg string, f inspectFlags, stdout, stderr io.Writer) error {
	...
	if err := runVulnScan(ctx, walkID, f.force, f.fresh, f.reachable, 1, false, false, f.goBinary, os.Getenv("USER"), "", f.policyPath, false, stderr, stderr); err != nil {
...
func runInspectGoMod(ctx context.Context, f inspectFlags, scope depScope, stdout, stderr io.Writer) error {
	...
	if walkID != "" {
		...
		if verr := runVulnScan(ctx, walkID, f.force, f.fresh, f.reachable, 1, false, false, f.goBinary, os.Getenv("USER"), filepath.Dir(f.gomodPath), f.policyPath, false, stderr, stderr); verr != nil {
