...
func TestRunWalkProject_ThreadsStdlibFromGoMod(t *testing.T) {
	...
	for _, want := range []bool{true, false} {
		t.Run(map[bool]string{true: "enabled", false: "disabled"}[want], func(t *testing.T) {
			...
			err := runWalkProject(context.Background(), gomodPath, false, true, 0, "", "", false, scopeComplete, walkdomain.WalkDepthFull, "", false, want, progress, uc, io.Discard, io.Discard)
			if err != nil {
