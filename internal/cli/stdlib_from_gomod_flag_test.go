...
func TestRunWalkProject_ThreadsStdlibFromGoMod(t *testing.T) {
	...
	for _, want := range []bool{true, false} {
		t.Run(map[bool]string{true: "enabled", false: "disabled"}[want], func(t *testing.T) {
			...
			_, err := runWalkProject(context.Background(), gomodPath, false, true, 0, "", "", false, scopeComplete, walkdomain.WalkDepthFull, "", false, want, progress, uc, nil, io.Discard, io.Discard)
			if err != nil {
