// Package vendortree adapts the vendor bounded context's scanner onto the
// godebug context's VendoredModuleLister port, so a directive found in a
// vendored file can name the module that published it without the godebug
// context learning how modules.txt is read.
//
// There is deliberately one parser of vendor/modules.txt in the repo. godebug
...
//
// The scan runs vendor-only: attributing a directive must never reach the
