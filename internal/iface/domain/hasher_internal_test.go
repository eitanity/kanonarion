...
func TestMarshalCanonical_MarshalFailure(t *testing.T) {
	...
	_, err := InterfaceRecordHasher{}.SetContentHash(InterfaceRecord{})
	if err == nil {
	...
	if !strings.Contains(err.Error(), "canonical interface record") {
...
func TestVerifyContentHash_MarshalFailure(t *testing.T) {
	...
	err := InterfaceRecordHasher{}.VerifyContentHash(InterfaceRecord{})
	if !errors.Is(err, injected) {
